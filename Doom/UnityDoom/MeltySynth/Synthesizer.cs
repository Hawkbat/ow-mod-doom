    ...
    public sealed class Synthesizer : IAudioRenderer
    {
        ...
        public Synthesizer(SoundFont soundFont, SynthesizerSettings settings)
        {
            ...
            foreach (var preset in soundFont.PresetArray)
            {
                ...
                if (!presetLookup.ContainsKey(presetId))
            ...
            System.Diagnostics.Debug.Assert(defaultPreset != null);
        ...
        public void Render(Span<float> left, Span<float> right)
        {
            ...
            while (wrote < left.Length)
            {
                ...
                new Span<float>(blockLeft, blockRead, rem).CopyTo(left.Slice(wrote, rem));
                new Span<float>(blockRight, blockRead, rem).CopyTo(right.Slice(wrote, rem));
